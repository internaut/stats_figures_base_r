...
You can find the [source code repository for this project on GitHub](https://github.com/internaut/stats_figures_base_r/). The code is licensed under [Apache License 2.0](https://www.apache.org/licenses/LICENSE-2.0). See [LICENSE](https://github.com/internaut/stats_figures_base_r/blob/master/LICENSE) file.
...
# same as above in order to save the figure as intro image
png("docs/front.png", 1200, 800, pointsize = 24)
plot(d, main = paste(n, "randomly sampled values from the standard normal distribution"),
     xlab = "x", ylab = "Density")
points(x, y = runif(n, 0, max_y), col = rgb(0, 0, 0, 0.5))
dev.off()
